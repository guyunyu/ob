	...
	show: function( json ) {
		if(json.status === 'success') {
			...
			if(info['t.disabled'] === 'N') {
				...
				ob.pages.item.container.find('.ob-item .detail .price').show();
				ob.pages.item.container.find('.ob-item .detail .spec').show();
			} else {
				ob.pages.item.container.find('.ob-item .detail .price').hide();
				ob.pages.item.container.find('.ob-item .detail .spec').hide();
				ob.pages.item.container.find('.ob-item .detail').show();
				ob.pages.item.container.find('.ob-item .detail').append('<div><span>item is not available for sale.</span></div>');
			}
		} else {
